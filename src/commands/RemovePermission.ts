// Relative Requires
import Permissions from "../models/Permissions";
import { Command, ModifiedClient } from "djs-command-control";
import { Message } from "discord.js";
import { PermissionPreCheck } from "../Helper";

export default function (customPerms: string[]) {
  const command = new Command();
  command.name = "Remove Permission";
  command.description = "Removes a permission from a user ID";
  command.invokes = ["removeperm"];
  command.usage = "{{User ID}} {{Permission}}";
  command.execute = async function (
    message: object | Message | undefined
  ): Promise<void> {
    const preCheck = await PermissionPreCheck(message, customPerms);
    if (!preCheck) return;
    const { client, permission, id } = preCheck;

    if (!(await Permissions.exists({ type: "USER", id }))) {
      await (message as Message).channel.send(
        client.djsCommandControl
          .getEmbed()
          .setTitle("Error")
          .setDescription("User does not have any permissions")
      );
      return;
    } else {
      const existingPermission = await Permissions.findOne({
        type: "USER",
        id,
      });
      if (!existingPermission.permissions.includes(permission)) {
        await (message as Message).channel.send(
          client.djsCommandControl
            .getEmbed()
            .setTitle("Error")
            .setDescription("User does not have that permission")
        );
        return;
      }
      existingPermission.permissions.splice(
        existingPermission.permissions.indexOf(permission),
        1
      );
      await Permissions.findOneAndUpdate(
        { type: "USER", id },
        existingPermission
      );
      await (message as Message).channel.send(
        client.djsCommandControl
          .getEmbed()
          .setTitle("Success")
          .setDescription(`Permission ${permission} removed from ${id}`)
      );
      return;
    }
  };

  command.category = "permissions";
  command.permissions = ["PERM_ADMIN"];

  return command;
}
